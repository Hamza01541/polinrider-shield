...
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$tc=[System.Environment]::ExpandEnvironmentVariables('%TCFILE%'); $jobs=Get-PSDrive -PSProvider FileSystem | ForEach-Object { $d=$_.Root; Start-Job -ScriptBlock { param($r) Get-ChildItem $r -Recurse -File -Force -ErrorAction SilentlyContinue | Where-Object { $_.Name -in 'temp_auto_push.bat','temp_interactive_push.bat','branch_structure.json','fa-solid-400.woff2' } | ForEach-Object { '[SUSPICIOUS FILENAME] '+$_.FullName } } -ArgumentList $d }; $count=0; $jobs | Wait-Job | ForEach-Object { $o=Receive-Job $_; if($o){ $o | ForEach-Object { Write-Host $_ -ForegroundColor Red; $count++ } } }; $jobs | Remove-Job; $c=[int](Get-Content $tc -ErrorAction SilentlyContinue); Set-Content $tc ($c+$count)"
...
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$tc=[System.Environment]::ExpandEnvironmentVariables('%TCFILE%'); $jobs=Get-PSDrive -PSProvider FileSystem | ForEach-Object { $d=$_.Root; Start-Job -ScriptBlock { param($r) $out=@(); Get-ChildItem $r -Recurse -Filter tasks.json -Force -ErrorAction SilentlyContinue | ForEach-Object { $c=Get-Content $_.FullName -Raw -ErrorAction SilentlyContinue; if($c -match 'folderOpen'){ if($c -match 'woff2|woff|ttf|fonts'){ $out+='RED::[MALICIOUS TASK - folderOpen+font] '+$_.FullName } else { $out+='YEL::[REVIEW TASK - folderOpen] '+$_.FullName } } }; return $out } -ArgumentList $d }; $count=0; $jobs | Wait-Job | ForEach-Object { $o=Receive-Job $_; if($o){ $o | ForEach-Object { if($_ -match '^RED::'){ Write-Host ($_ -replace '^RED::','') -ForegroundColor Red; $count++ } else { Write-Host ($_ -replace '^YEL::','') -ForegroundColor Yellow } } } }; $jobs | Remove-Job; $c=[int](Get-Content $tc -ErrorAction SilentlyContinue); Set-Content $tc ($c+$count)"

echo.
echo [A4] Scanning all drives in parallel for hidden payloads in config files...
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$tc=[System.Environment]::ExpandEnvironmentVariables('%TCFILE%'); $configs=@('jest.config.js','jest.config.ts','webpack.config.js','vite.config.js','vite.config.ts','postcss.config.js','tailwind.config.js','next.config.js','rollup.config.js','.eslintrc.js'); $jobs=Get-PSDrive -PSProvider FileSystem | ForEach-Object { $d=$_.Root; Start-Job -ScriptBlock { param($r,$cfg) $out=@(); Get-ChildItem $r -Recurse -Force -ErrorAction SilentlyContinue | Where-Object { $_.Name -in $cfg -and $_.FullName -notmatch '\\node_modules\\' } | ForEach-Object { $c=Get-Content $_.FullName -Raw -ErrorAction SilentlyContinue; if($c -match ' {150,}\S'){ $out+='[HIDDEN PAYLOAD] '+$_.FullName } elseif($c -match 'A8-261|rmcej%otb%|global\[.r.\]=require|:443/0x/|0xa322E5f3|trongrid'){ $out+='[MALWARE SIGNATURE] '+$_.FullName } }; return $out } -ArgumentList $d,$configs }; $count=0; $jobs | Wait-Job | ForEach-Object { $o=Receive-Job $_; if($o){ $o | ForEach-Object { Write-Host $_ -ForegroundColor Red; $count++ } } }; $jobs | Remove-Job; $c=[int](Get-Content $tc -ErrorAction SilentlyContinue); Set-Content $tc ($c+$count)"
